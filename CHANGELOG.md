...
- Compile and review gates now surface visible per-gate token savings in stdout using the shared `hybrid_text_v1` telemetry estimate. The new `[token-economy] saved ~N tokens ...` line appears only when compaction actually reduces output, while no-op and tiny same-line reductions stay silent to avoid noise.
...
- Documented the fix-or-defer-before-done review rule in workflow, task-contract, and review-template guidance.
