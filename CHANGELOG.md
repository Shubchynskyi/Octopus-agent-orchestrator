...
## 2.4.3
- carried optional plan metadata through task-mode, build-review-context, and completion artifacts: `enter-task-mode` now accepts `--plan-path` pointing to an approved task-plan artifact; when supplied the gate validates the plan (approved status, matching task_id, sha256 integrity), computes `plan_path`/`plan_sha256`/`plan_summary`, and embeds them in the task-mode artifact under a `plan` object; `build-review-context` reads plan metadata from task-mode evidence and surfaces `plan.plan_guided`/`plan_path`/`plan_sha256`/`plan_summary` so reviewers can see whether the task is plan-guided or freeform; `completion-gate` surfaces the same plan evidence in its output and formatter; all plan fields are optional — absent plans produce `plan: null` with `plan_guided: false`, preserving today's freeform behavior; TASK_MODE_ENTERED timeline event and metrics also carry plan-guided flag and plan coordinates
- added optional `task-plan` artifact schema and validator for the planner/executor split workflow: new `src/schemas/task-plan.ts` exports a JSON Schema (`taskPlanSchema`), a runtime validator (`validateTaskPlan`), a digest helper (`computeTaskPlanDigest`), a serializer (`serializeTaskPlan`), and an `isApprovedPlan` predicate; the plan artifact is fully opt-in — if a validated plan exists the executor follows it, otherwise task execution continues unchanged; schema enforces required fields (`schema_version`, `task_id`, `status`, `goal`, `scope_files`, `risk_level`, `steps`), step-id uniqueness, `depends_on` referential integrity, and forward-compatible version gating; artifact path convention is `<bundle>/runtime/reviews/<task-id>-task-plan.json`
- made deployed bundle directory name configurable: added `resolveBundleName()` utility in `core/constants` that checks `OCTOPUS_BUNDLE_NAME` env var and falls back to `Octopus-agent-orchestrator`; added `--bundle-name` global CLI flag that sets the env var before command dispatch; migrated all path-construction and generated-content sites across ~30 source files from hardcoded `DEFAULT_BUNDLE_NAME` constant to the resolver function; module-level constants that referenced the bundle name (command templates, rule contract migrations, entrypoint rule links, gitignore entries, allow-list entries) converted to lazy getter functions to ensure correct evaluation after env var is set; `bin/octopus.ts` launcher also reads `OCTOPUS_BUNDLE_NAME` when locating the deployed bundle root; existing default unchanged, feature is explicit opt-in
- added `NOTICE` (Apache 2.0 attribution notice) and `TRADEMARKS.md` (trademark usage policy) for legal and branding governance hygiene; `NOTICE` provides the required copyright/attribution text per Apache License Section 4(d); `TRADEMARKS.md` documents permitted and restricted use of the project name, logo, CLI aliases, and third-party marks; both files are included in the published npm package via `package.json` `files`
- added `docs/compatibility-matrix.md` publishing an evidence-backed provider compatibility matrix across all 8 provider families (Claude, Codex, Gemini, Qwen, GitHub Copilot, Windsurf, Junie, Antigravity); covers core feature support (entrypoint materialization, managed blocks, token economy, compact-command protocol), gate sequence compliance, reviewer delegation modes (delegated sub-agent / conditional / same-agent fallback), review type availability, and per-provider test coverage with traceability to ~194 execution-path tests and supporting test suites; status badges distinguish tested, partial, and experimental capabilities
- added `docs/threat-model.md` documenting trust surfaces, threat actors, and mitigations for the install, update, write (materialization/runtime artifacts), and trust/verification layers; covers supply-chain risks, same-user trust boundary limitations, defense-in-depth summary, and residual risk acceptance; complements `SECURITY.md` and the control-plane isolation docs
- added `.github/CODEOWNERS` mapping orchestrator protected paths, CI workflows, security config, agent entrypoints, and package metadata to designated code-owner teams; path patterns mirror the local protected-path enforcement list so the same files guarded at task-completion time are also guarded at PR-merge time; `docs/branch-protection.md` documents recommended GitHub branch-protection settings (required checks, required reviews, CODEOWNERS integration, merge restrictions) and explains how local and PR-time governance layers complement each other
- added CycloneDX SBOM generation workflow (`.github/workflows/sbom.yml`): generates a CycloneDX 1.5 JSON Software Bill of Materials from `package-lock.json` on push, PR, version tags, and manual dispatch using `@cyclonedx/cyclonedx-npm` with reproducible output; uploads the SBOM as a GitHub Actions artifact (`sbom-cyclonedx`) with 90-day retention; `docs/sbom.md` documents CI integration, local generation/validation commands, output format, and troubleshooting
- added repository secret scanning with gitleaks: new `.github/workflows/secret-scanning.yml` CI workflow runs gitleaks on push, PR, weekly schedule, and manual dispatch against the full commit history; `.gitleaks.toml` configures project-specific allowlists for test fixtures, runtime artifacts, and placeholder tokens; `docs/secret-scanning.md` documents CI integration, local install/run commands, pre-commit hook setup, configuration, and troubleshooting; complements the runtime redaction layer (T-014) with a prevention gate
- added scheduled smoke matrix workflow (`.github/workflows/smoke-schedule.yml`): runs the full cross-platform lifecycle smoke suite (ubuntu, windows, macos) on a weekly cron schedule (Sundays 04:15 UTC) and on `workflow_dispatch` to detect runner image drift, Node.js default-version changes, and environment regressions when the repo is quiet; includes the same failure-forensics artifact upload as the main CI smoke job
- added global `--no-color` CLI flag: when passed, sets `NO_COLOR=1` in the process environment before command dispatch so all downstream color helpers produce plain text; complements the existing `NO_COLOR` and `FORCE_COLOR` environment variable support; `--no-color` is extracted as a global flag before command routing and does not interfere with per-command option parsing; help text updated in both `printHelp` and `buildHelpText`
